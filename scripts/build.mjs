...
async function createBundle(target) {
  ...
  const bundle = ['// Generated by scripts/build.mjs for file:// portable usage.', guides, classGuides, classBuildGuides, specGuides, dungeons, raids, professions, farming, assets, subscriptions, filters, app].join('\n\n');

  await writeFile(path.join(target, 'src/app.bundle.js'), bundle, 'utf8');
}

async function createWindowsData(target) {
  const [guidesModule, classModule, buildModule, specModule, dungeonModule, professionModule, farmingModule, subscriptionModule, assetModule] = await Promise.all([
